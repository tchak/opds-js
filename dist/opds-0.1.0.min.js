...
OPDS.Feed=_.Class({initialize:function(a){this.browser=a||new OPDS.Support.Browser;this.rawDoc=null},extend:{parseUrl:function(a,b,d,c){d=d||new OPDS.Support.Browser;var f=this;d.goTo(a,function(e){if(e.isOk()){var g=f.parseRaw(e.body(),c,e);if(g==null){e.discover(e.currentLocation);b.call(e,false)}else b.call(e,g)}else b.call(e,false)})},parseRaw:function(a,b,d){return(new OPDS.Parser(b)).parse(a,d)},fromJQuery:function(a,b){var d=new OPDS.Feed(b);d.rawDoc=a;d.serialize();return d}},serialize:function(){this.id=
this.rawDoc.find("feed>id").text();this.icon=this.rawDoc.find("feed>icon").text();this.title=this.rawDoc.find("feed>title").text();this.author={name:this.rawDoc.find("feed>author>name").text(),uri:this.rawDoc.find("feed>author>uri").text(),email:this.rawDoc.find("feed>author>email").text()};this.entries=_(this.rawDoc.find("feed>entry")).chain().toArray().map(function(a){return OPDS.Entry.fromJQuery($(a),this.browser)},this).value();OPDS.Support.LinkSet.extract(this,"feed>link")},nextPageUrl:function(){},
prevPageUrl:function(){},isPaginated:function(){},isFirstPage:function(){return this.isPaginated()?!this.prevPageUrl():false},isLastPage:function(){return this.isPaginated()?!this.nextPageUrl():false},nextPage:function(){return Feed.parseUrl(this.nextPageUrl(),this.browser)},prevPage:function(){return Feed.parseUrl(this.prevPageUrl(),this.browser)},inspect:function(){}});OPDS.NavigationFeed=_.Class(OPDS.Feed,{});OPDS.AcquisitionFeed=_.Class(OPDS.Feed,{});
OPDS.Support.Browser=_.Class({goTo:function(a,b){var d=(new URI(a)).toString(),c=this;this.lastResponse=null;this.currentLocation=d;try{$.get(d,function(g,h,i){c.lastResponse=i;b.apply(c,[c])})}catch(f){if(jQuery.browser.msie&&window.XDomainRequest){var e=new XDomainRequest;e.open("get",d);e.onload=function(){c.lastResponse=this;b.apply(c,[c])};e.send()}else console.log("An error occured, or your browser is to old for this...")}},isOk:function(){return this.status()==200},status:function(){return this.lastResponse?
this.lastResponse.status:null},headers:function(){return this.lastResponse?this.lastResponse.getAllResponseHeaders():null},body:function(){return this.lastResponse?this.lastResponse.responseText:null},discover:function(a){this.goTo(a);if(this.isOk()){a={rawDoc:$(this.body())};OPDS.Support.LinkSet.extract(a,'[type="application/atom+xml;type=entry;profile=opds-catalog"], [type="application/atom+xml;profile=opds-catalog"]');if(a.links.size()==0)return false;return a.links}else return false}});
OPDS.Support.Link=_.Class({initialize:function(a,b){this.browser=b||new OPDS.Support.Browser;if(this.browser.currentLocation)a[1]=URI.join(this.browser.currentLocation,a[1]).toString();this.rel=a[0]||null;this.url=a[1]||null;this.title=a[2]||null;this.type=a[3]||null;this.price=a[4]||null;this.currency=a[5]||null},navigate:function(a){return OPDS.Feed.parseUrl(this.url,a,this.browser)},inspect:function(){}});
OPDS.Support.LinkSet=_.Class({initialize:function(a){this.browser=a||new OPDS.Support.Browser;this.store=[];this.store.rel_store={};this.store.txt_store={};this.store.lnk_store={};this.store.typ_store={}},extend:{extract:function(a,b){a.links=new OPDS.Support.LinkSet(a.browser);a.rawDoc.find(b).each(function(d,c){var f=null,e=$(c);if(e.attr("title"))f=e.attr("title");var g=e.attr("href"),h=e.attr("type")?e.attr("type"):null;e.attr("rel")?_.each(e.attr("rel").split(),function(i){a.links.push(i,g,f,
h)}):a.links.push(null,g,f,h)})}},set:function(a,b){var d=new OPDS.Support.Link([a].concat(b),this.browser),c=this.store;c.push(d);d=c.length-1;c.rel_store[a]||(c.rel_store[a]=[]);c.rel_store[a].push(d);c.txt_store[b[1]]||(c.txt_store[b[1]]=[]);c.txt_store[b[1]].push(d);c.lnk_store[_.first(b)]||(c.lnk_store[_.first(b)]=[]);c.lnk_store[_.first(b)].push(d);c.typ_store[_.last(b)]||(c.typ_store[_.last(b)]=[]);c.typ_store[_.last(b)].push(d)},get:function(a){return this.__remap(this.store.rel_store[a])},
each:function(a){_.each(this.store,a);return this},push:function(a,b,d,c){this.set(a,[b,d,c])},linkUrl:function(){},linkRel:function(){},linkText:function(){},linkType:function(){},size:function(){return this.store.length},by:function(a){var b={};_.each(this.__collection(a),function(d,c){return b[c]=this.__remap(d)},this);return b},links:function(){return _.keys(this.store.lnk_store)},rels:function(){return _.keys(this.store.rel_store)},texts:function(){return _.keys(this.store.txt_store)},inspect:function(){},
first:function(){return _.first(this.store)},last:function(){return _.last(this.store)},__collection:function(a){switch(a){case "link":return this.store.lnk_store;case "rel":return this.store.rel_store;case "txt":return this.store.txt_store;case "type":return this.store.typ_store}},__remap:function(a){if(!a||a.length==0)return null;return _.map(a,function(b){return this.store[b]},this)}});
